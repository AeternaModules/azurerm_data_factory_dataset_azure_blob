...
EOT
  ...
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_azure_blobs : (
  ...
  validation {
    condition = alltrue([
      for k, v in var.data_factory_dataset_azure_blobs : (
        v.filename == null || (length(v.filename) > 0)
      )
    ])
    error_message = "must not be empty"
  }
  ...
  # --- Unconfirmed validation candidates, derived from azurerm_data_factory_dataset_azure_blob's provider source ---
